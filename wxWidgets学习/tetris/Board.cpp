...
void Board::linesRemovedChanged(int numLines) 
{
...
void Board::OnPaint(wxPaintEvent& event) 
{
    ...
    int boardTop = size.GetWidth() - BoardHeight * SquareHeight();
    
    for (int i = 0; i < BoardHeight; i++) {
        for (int j = 0; j < BoardWidth; j++) {
            Tetrominoes shape = ShapeAt(j, BoardHeight - i - 1);
            if (shape != NoShape) {
                DrawSquare(dc, j * SquareWidth(), boardTop + i * SquareHeight(), shape);
            }
        }
    }
}
...
void Board::DrawSquare(wxPaintDC& dc, int x, int y, Tetrominoes shape) 
{
    ...
    dc.DrawLine(x + 1, y + SquareHeight() - 1, x + SquareWidth() - 1,
        y + SquareHeight() - 1);
    dc.DrawLine(x + SquareWidth(), y + SquareHeight() - 1, 
        x + SquareWidth() - 1, y +  1);
